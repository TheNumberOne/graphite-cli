...
export const handler = async (argv: argsT): Promise<void> => {
  return profile(argv, canonical, async (context) => {
    ...
    const filePath = path.join(tmpDir.name, 'meta');
    fs.writeFileSync(filePath, metaString);
    gpExecSync(
      {
        command: `${argv['text-editor']} "${filePath}"`,
    ...
    return writeMetadataRef(argv.branch, fs.readJSONSync(filePath));
